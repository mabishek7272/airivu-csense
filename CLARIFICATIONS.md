...
| 26 | Resend credentials | Outbound email needs an API key and a verified sending domain. | **NEEDS YOUR ACTION.** Set `RESEND_API_KEY` and `RESEND_FROM_ADDRESS` in `.env`. In Cloudflare, add the SPF/DKIM/DMARC records Resend provides for the sending domain, and configure Email Routing to forward replies to the inbound webhook (that webhook is not built yet — it is the remaining half of reply-to-acknowledge). Until the key is set, the email channel reports itself as unconfigured in the Developer Console rather than failing silently. |
