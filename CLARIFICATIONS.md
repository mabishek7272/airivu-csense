...
| 15 | YOLOv8 licence | 5 migrated models are Ultralytics YOLOv8 weights under **AGPL-3.0**. | **DECIDED 2026-08-26 (owner):** these models have been in production use since 2022; the owner accepts the position and directed that all be wired up. Licence terms stay recorded on each version (`license_metadata`) so the facts remain visible if the position is ever revisited. **Reconfirmed 2026-09-02 (owner):** directed that weights retrained on the owner's own data be used and wired in on the same basis. Retraining does not itself change the licence position - AGPL-3.0 attaches to the Ultralytics architecture/training code, not cured by different weights - so this is the same accepted position, not a new legal fact; noted so the reasoning doesn't need rediscovering. No further action pending. |
...
| 30 | Legacy password migration | Legacy `csense_users.db` stores bcrypt hashes with no way to verify they were never compromised, and re-hashing would mean trusting a password we never chose the scheme for. Re-hash the legacy bcrypt hash into the new Argon2id scheme, or force every legacy user to set a brand-new password? | **DECIDED 2026-09-09 (owner): force-reset.** No legacy hash is migrated or read into memory for any purpose beyond skipping over it. Every migrated user lands in the new system as `status='invited'`, `password_hash=NULL` — the exact shape a normal team invitation already produces — and is sent a real invitation link through that same, already-audited flow rather than a new "password reset" feature. `login()`'s own dummy-hash comparison already refuses a null-password account correctly, so this needed zero new authentication code. See `backend/migrations/import_legacy_users.py`. **The real invitation emails are a separate, still-pending step** — deliberately not sent automatically; that's the account owner's own call to trigger. |
