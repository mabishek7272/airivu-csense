 ...
 *  no models while the request is still in flight, and they stop looking. So `data` stays
 *  null until something has actually arrived, and callers branch on loading first.
 *
 *  A refresh keeps the previous data on screen rather than flashing back to a skeleton -
 *  re-fetching after a promotion should not blank the list the promoted row is in.
 *
 *  Verbatim port of frontend/customer-crm/src/hooks/useResource.ts - framework-agnostic,
...
export interface ResourceState<T> {
  ...
  refreshing: boolean;
  reload: () => void;
  /** Applies a local change without a round trip - used after a promotion so the list
   *  updates immediately and the server call confirms it rather than gating it. */
