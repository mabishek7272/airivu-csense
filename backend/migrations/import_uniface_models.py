"""Imports the clean-licensed subset of the `uniface` model zoo into the model registry.

    python import_uniface_models.py --source /path/to/staged/models [--dry-run]

Sibling of `import_legacy_models.py`, adapted for a different provenance shape (a
download URL, not a legacy host path) and a different default state: every model here
lands `uploaded`, not `validated` - see `uniface_model_manifest.py`'s own docstring for
why (these have no production history with this business, unlike the legacy estate).

Same three steps, same idempotency guarantee:
...
import psycopg
from uniface_model_manifest import UNIFACE_MODELS, LegacyModel
...
def _upsert_model(cur, model: LegacyModel) -> str:
    ...
    cur.execute(
        ...
        VALUES (%s, %s, %s, 'uniface-import', 'active', %s)
        RETURNING id
        """,
...
def _register_version(cur, model: LegacyModel, model_id: str, upload, local_path: Path) -> str | None:
    ...
    provenance = {
        ...
        "source_url": model.legacy_paths[0] if model.legacy_paths else None,
...
def main() -> None:
    ...
    with psycopg.connect(_dsn()) as conn, conn.cursor() as cur:
        for model, path in verified:
            ...
            if version_id is None:
                ...
                imported += 1
                flag = "  [BIOMETRIC]" if model.access_classification == "biometric" else ""
    ...
    if biometric:
        print(
            f"\n{len(biometric)} model(s) produce biometric embeddings and were registered with "
            "access_classification='biometric' - the model.promote endpoint will require "
            "acknowledge_biometric=true before any of them can reach a deployable state."
        )
    print(
