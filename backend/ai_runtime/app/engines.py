...
# cross-check" entry), plus the 2 resolved from the same public reference on 2026-09-16.
# Dispatched by model_name for the same reason as everything else in this function:
# task_code alone is ambiguous or shared for at least some of these (`face_attribute` is
# shared between `uniface-fairface-attributes` and `uniface-faceattribnet-attributes`, two
# models whose "attributes" are entirely different things; `face_landmark` is shared with
# `uniface-facemesh-landmark`, a different model with a different output shape, decoded
# separately above).
_UNIFACE_ENGINES_BY_MODEL_NAME: dict[str, type] = {
    ...
    # Resolved from the same public reference, 2026-09-16 (previously the 2 "no-guess"
    # models). `face_parsing` is unique to bisenet, but it is dispatched by model_name
    # anyway, for the same reason as everything else in this table.
