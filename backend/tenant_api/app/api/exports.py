...
download"). One export type this pass: incidents, as CSV. `POST` returns a `queued` job
immediately; the actual query + CSV build + MinIO upload runs in a FastAPI
...
is still open when the task starts; `request_incidents_export` commits it explicitly
before scheduling the task so the freshly-inserted row is actually visible to the task's
own, separate session (see its own comment).
...
from csense_shared.exports import build_incidents_csv
