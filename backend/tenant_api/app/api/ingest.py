...
Authentication is the standard customer token carrying `detection.ingest`, which the
`edge_device` role holds and nothing else. That is an interim arrangement: Phase 3 replaces
it with per-device mutual TLS issued at enrolment (see migration 0019).
"""
...
async def ingest(
    ...
    """
    require_permission(context, "detection.ingest")
    ...
    site_id, timezone_name = await _camera_site(
        db, tenant_id=context.tenant_id, camera_id=body.camera_id
    )
